...
// Helper function to create AI context info
function createAIContextInfo(aimode = true) {
    ...
    return {
        ...
        externalAdReply: {
            title: 'AI Assistant',
            body: 'Powered by AI',
            thumbnailUrl: 'https://i.ibb.co/2M7Rt6M/ai-icon.png',
            mediaType: 1,
...
async function showHelp(sock, chatId, reply) {
    const helpText = `🔘 *Button & AI Commands*\n\n` +
                    ...
                    `*7. AI Mode Control*\n` +
                    `\`.button ai on\` - Enable AI mode (shows AI tag)\n` +
...
async function handleNativeButtons(sock, chatId, text, quotedMsg, reply) {
    ...
    const payload = {
        ...
        buttons: buttons,
        contextInfo: createAIContextInfo(global.aiMode.get(chatId) || false)
    };

    console.log('📦 Payload:', JSON.stringify(payload, null, 2));
    console.log(`🤖 AI Mode: ${global.aiMode.get(chatId) || false}`);
...
// 6. List Button - USING SIMPLE BUTTONS
async function handleListButton(sock, chatId, text, quotedMsg, reply) {
    console.log('\n📝 List Button (as multiple buttons):');
    ...
    await sendButtons(sock, chatId, payload, { quoted: quotedMsg });
    console.log('✅ List as multiple buttons sent');
}
...
async function handleAIMode(sock, chatId, text, quotedMsg, reply) {
    ...
    if (mode === 'on' || mode === 'enable' || mode === 'true' || mode === '1') {
        ...
        await sendButtons(sock, chatId, {
            ...
            buttons: buttons,
            contextInfo: createAIContextInfo(true)
        }, { quoted: quotedMsg });
        ...
        await sendButtons(sock, chatId, {
            ...
            contextInfo: createAIContextInfo(false)
        }, { quoted: quotedMsg });
        ...
        await sendButtons(sock, chatId, {
            ...
            footer: 'AI Assistant',
            buttons: buttons,
            contextInfo: createAIContextInfo(global.aiMode.get(userId))
        }, { quoted: quotedMsg });
