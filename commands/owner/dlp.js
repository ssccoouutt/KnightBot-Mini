            ...
            try {
                ...
                for (const format of formats) {
                    if (format.vcodec !== 'none') {
                        ...
                        // Get file size from various possible fields
                        let fileSize = format.filesize || format.filesize_approx || 0;
                        
                        // If still no size, try to calculate from bitrate
                        if (fileSize === 0 && format.tbr && format.tbr > 0 && info.duration) {
                            // Approximate size: bitrate * duration / 8
                            fileSize = Math.floor(format.tbr * 1000 * info.duration / 8);
                        }
                        ...
                        } else if (qualities.has(quality) && fileSize > (qualities.get(quality)?.filesize || 0)) {
                            // Update with larger file size for same quality
                            const existing = qualities.get(quality);
                            existing.filesize = fileSize;
                            qualities.set(quality, existing);
                        }
                    }
                }
                ...
                qualities.set('mp3', {
                    ...
                    height: 0,
                    ext: 'mp3',
                    filesize: audioSize > 0 ? audioSize : null,
                    vcodec: 'none',
                    acodec: 'mp4a'
                });
            ...
            if (buttonId && buttonId.startsWith('dlp_qual_')) {
                ...
                if (!isNaN(index) && index >= 0 && index < qualities.length) {
                    ...
                    try {
                        ...
                        const useDocument = fileSizeMB > 200;
