 ...
 * Ensures video files have audio before sending
 */
...
// Download video with audio included (using best format that has both)
async function downloadVideoWithAudio(url, qualityInfo) {
    ...
    return new Promise((resolve, reject) => {
        ...
        let formatSpec;
        if (qualityInfo.height) {
            formatSpec = `best[height<=${qualityInfo.height}][ext=mp4]/best[height<=${qualityInfo.height}]/best`;
        } else {
            formatSpec = 'best[ext=mp4]/best';
        }
        
        const cmd = `yt-dlp ${cookieArg} -f "${formatSpec}" -o "${outputPath}" "${url}"`;
        ...
        exec(cmd, { maxBuffer: 500 * 1024 * 1024 }, async (error, stdout, stderr) => {
            ...
            if (stats.size < 50000) {
                reject(new Error(`File too small (${stats.size} bytes) - video may be unavailable`));
                return;
            }
            
            // Verify the file has audio
            const hasAudio = await hasAudioStream(outputPath);
            console.log(`[DLP] File has audio: ${hasAudio}`);
            
            if (!hasAudio) {
                console.log('[DLP] Downloaded file has no audio, trying alternative format...');
                // Try alternative download method
                const altCmd = `yt-dlp ${cookieArg} -f "bestvideo[height<=${qualityInfo.height}]+bestaudio/best" --merge-output-format mp4 -o "${outputPath}" "${url}"`;
                
                exec(altCmd, { maxBuffer: 500 * 1024 * 1024 }, async (altError, altStdout, altStderr) => {
                    if (altError) {
                        reject(new Error('Video downloaded but has no audio, and audio merge failed'));
                        return;
                    }
                    
                    const altHasAudio = await hasAudioStream(outputPath);
                    if (!altHasAudio) {
                        reject(new Error('Video file has no audio stream'));
                        return;
                    }
                    
                    const altStats = fs.statSync(outputPath);
                    resolve({ path: outputPath, filename: 'video.mp4', size: altStats.size, tempDir: tempDir, hasAudio: true });
                });
                return;
            }
            
            resolve({ path: outputPath, filename: 'video.mp4', size: stats.size, tempDir: tempDir, hasAudio: true });
        });
    });
}
...
async function downloadAudioOnly(url) {
    ...
    return new Promise((resolve, reject) => {
        ...
        exec(cmd, { maxBuffer: 500 * 1024 * 1024 }, (error, stdout, stderr) => {
            ...
            resolve({ path: audioFile, filename: files[0], size: stats.size, tempDir: tempDir, isAudio: true });
        });
    });
}
...
async function getAvailableQualities(url) {
    return new Promise((resolve, reject) => {
        ...
        exec(cmd, { maxBuffer: 10 * 1024 * 1024 }, (error, stdout, stderr) => {
            if (error) { reject(new Error('Failed to fetch video info')); return; }
                ...
                for (const format of formats) {
                    if (format.vcodec !== 'none') {
                        ...
                        if (!qualities.has(quality) || height > (qualities.get(quality)?.height || 0)) {
                            qualities.set(quality, { formatId: format.format_id, height: height, ext: format.ext, filesize: format.filesize });
                        }
                    }
                }
                
                qualities.set('mp3', { formatId: 'bestaudio/best', height: 0, ext: 'mp3', filesize: null, isAudio: true });
                
                const qualityOrder = ['4K', '2K', '1080p', '720p', '480p', '360p', '240p', 'mp3'];
                const sortedQualities = [];
                for (const q of qualityOrder) {
                    if (qualities.has(q)) { sortedQualities.push({ name: q, ...qualities.get(q) }); }
                }
                
                resolve({ title: info.title, duration: info.duration, thumbnail: info.thumbnail, uploader: info.uploader, qualities: sortedQualities });
...
module.exports = {
    ...
    async execute(sock, msg, args, context) {
        ...
        const url = args[0];
        if (!url.startsWith('http')) { return reply(`❌ Please provide a valid URL`); }
        
        await react('🔍');
        const processingMsg = await reply(`🔍 *Analyzing URL...*\n\n${url}`);
        
        try {
            if (!cookiesPath || !fs.existsSync(cookiesPath)) { await downloadCookies(); }
            ...
            for (const sess of existingSessions) { if (sess.command === 'dlp') { sessionManager.clearSession(sess.id); } }
            
            const session = sessionManager.createSession(sender, from, 'dlp', { url: url, videoInfo: videoInfo });
            const sessionId = session.id.split(':').pop();
            
            if (videoInfo.thumbnail) {
                try {
                    await sock.sendMessage(from, { image: { url: videoInfo.thumbnail }, caption: `🎬 *${videoInfo.title || 'Video'}*\n⏱️ ${formatDuration(videoInfo.duration)}\n👤 ${videoInfo.uploader || 'Unknown'}` }, { quoted: msg });
                } catch (e) {}
            }
            ...
            for (let i = 0; i < videoInfo.qualities.length; i++) {
                ...
                let btnText = q.name;
                if (q.filesize) { btnText += ` (${formatFileSize(q.filesize)})`; }
                buttons.push({ id: `dlp_qual_${sessionId}_${i}`, text: btnText });
            }
            buttons.push({ id: 'cancel', text: '❌ Cancel' });
            
            await sock.sendMessage(from, { text: `✅ *Video Info:* ${videoInfo.title || 'Video'}\n📊 Available: ${videoInfo.qualities.length} options\n\nSelect quality:`, edit: processingMsg.key });
            const sentMsg = await sendButtons(sock, from, { text: `🎬 ${videoInfo.title || 'Select quality'}`, buttons: buttons, aimode: FORCE_AI_MODE }, {});
            sessionManager.addPendingMessage(sender, from, sentMsg.key.id, 'dlp');
            await react('✅');
            
        } catch (error) {
            await sock.sendMessage(from, { text: `❌ Error: ${error.message}`, edit: processingMsg.key });
            await react('❌');
        }
    },
    ...
    async handleSession(sock, msg, session, context) {
        ...
        if (isButtonClick) {
            ...
            else if (msg.message?.templateButtonReplyMessage) { buttonId = msg.message.templateButtonReplyMessage.selectedId; }
                ...
                if (index >= 0 && index < qualities.length) {
                    ...
                    try {
                        ...
                        if (isAudio) {
                            ...
                            result = await downloadVideoWithAudio(session.data.url, selected);
                        }
                        
                        const fileSizeMB = (result.size / (1024 * 1024)).toFixed(2);
                        const caption = `✅ *Download Complete!*\n\n🎬 ${session.data.videoInfo.title || 'Video'}\n📹 ${selected.name}\n📊 ${fileSizeMB} MB`;
                        
                        if (isAudio) {
                            await sock.sendMessage(from, { audio: fs.readFileSync(result.path), mimetype: 'audio/mpeg', caption: caption, ptt: false }, { quoted: msg });
                        } else {
                            // Double-check the file has audio before sending
                            const hasAudio = await hasAudioStream(result.path);
                            if (!hasAudio) {
                                await sock.sendMessage(from, { text: `⚠️ *Warning:* Video downloaded but has no audio.\n\nTry downloading audio-only (MP3) instead.`, edit: processingMsg.key });
                                // Still send the video but inform user
                            }
                            await sock.sendMessage(from, { video: fs.readFileSync(result.path), mimetype: 'video/mp4', caption: caption }, { quoted: msg });
                        }
                        
                        // Cleanup
                        try { fs.rmSync(result.tempDir, { recursive: true, force: true }); } catch (e) {}
                        
                        await sock.sendMessage(from, { text: `✅ Download complete!`, edit: processingMsg.key });
                        await react('✅');
                        sessionManager.clearSession(session.id);
                        
                    } catch (error) {
                        await sock.sendMessage(from, { text: `❌ Download failed: ${error.message}`, edit: processingMsg.key });
                        await react('❌');
                    }
                }
                return true;
