    ...
    async execute(sock, msg, args, context) {
        ...
        if (args.length === 0) {
            return reply(`🔄 *Clone Command*\n\n` +
                       ...
                       `• \`${config.prefix}clone https://drive.lora.com/drive/folders/xxx my-repo\``);
        }
        ...
        if (link.includes('drive.lora.com') || link.includes('drive/folder')) {
            try {
...
async function handleGitHubToDrive(sock, from, repoUrl, folderName, processingMsg, reply) {
    ...
    const driveLink = `https://drive.lora.com/drive/folders/${folderId}`;
